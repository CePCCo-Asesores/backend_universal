    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50 p-4">
      <div className="max-w-5xl mx-auto">
        ...
        <header className="text-center mb-8">
          ...
          <div className="mt-4 text-sm text-purple-600 font-medium">✨ Paso {userData.paso} de 7</div>
        </header>
        ...
        <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-3 text-center">
          <p className="text-sm text-blue-800">
            <strong>PRINCIPIO ND CLAVE:</strong> La neurodiversidad es una variación natural del cerebro humano. Nunca se
            trata como déficit. Toda propuesta es afirmativa, sensorialmente consciente y éticamente defendible.
          </p>
        </div>
      </div>
    </div>
...
function PasoUno({ userData, setUserData }: StepProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800 mb-3">¡Hola! 🧠 Soy tu Asistente de Planeación ND</h2>
        <p className="text-gray-600 mb-4">Mi misión es ayudarte a crear actividades que celebren y potencien la diversidad neurológica.</p>
        <p className="text-lg font-medium text-gray-800">¿Qué tipo de usuario eres?</p>
      </div>
...
function PasoDos({ userData, setUserData }: StepProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800 mb-3">¿Qué tipo de neurodiversidad está presente?</h2>
        <p className="text-gray-600 mb-4">Puedes elegir varias opciones. ¿Hay alguna prioritaria o prefieres enfoque integrado?</p>
      </div>
        ...
        {neurodiversidades.map((nd) => (
          <button
            ...
            className={`flex items-start p-4 border-2 rounded-xl transition-all duration-200 text-left ${
              neurodiversidadesSeleccionadas.includes(nd.id) ? 'border-purple-400 bg-purple-50 text-purple-800' : 'border-gray-200 hover:border-purple-300'
            }`}
          >
      ...
      {neurodiversidadesSeleccionadas.length > 0 && (
        <div className="flex justify-between items-center pt-4 border-t">
          <button onClick={() => setUserData({ ...userData, paso: 1 })} className="px-6 py-2 text-gray-600 hover:text-gray-800 transition-colors">
            ← Volver
          </button>
          <button onClick={continuar} className="px-8 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-medium">
            Continuar →
          </button>
        </div>
      )}
...
function PasoTres({ userData, setUserData }: StepProps) {
  ...
  const seleccionarOpcion = (opcionId: string) => {
    if (userData.tipoUsuario === 'docente' && opcionId === 'crear') {
      setUserData({ ...userData, opcionMenu: opcionId, paso: 3.5 })
    } else {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800 mb-3">Con esta información, ¿qué necesitas hacer hoy?</h2>
      ...
      <div className="flex justify-between items-center pt-4 border-t">
        <button onClick={() => setUserData({ ...userData, paso: 2 })} className="px-6 py-2 text-gray-600 hover:text-gray-800 transition-colors">
          ← Volver
        </button>
      </div>
    </div>
...
/* Paso 3.5: Activación de Planeación ND (solo docentes que eligen "crear") */
function PasoTresYMedio({ userData, setUserData }: StepProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800 mb-3">Activación de Planeación ND para Docentes</h2>
        <p className="text-gray-600 mb-4">Puedo ayudarte a generar una planeación ND completa. Responde estas preguntas:</p>
      </div>

      <div className="space-y-6">
        {[
          { id: 'grado', label: '1️⃣ 📚 ¿Cuál es el grado escolar al que va dirigida la planeación?', ph: 'Ej: 3° primaria, 1° secundaria, preescolar...' },
          { id: 'contenidoTematico', label: '2️⃣ 🧪 ¿Cuál es el contenido temático principal?', ph: 'Ej: Matemáticas, Ciencias Naturales, Lenguaje...' },
          { id: 'temaDetonador', label: '3️⃣ 🎯 ¿Cuál es el tema detonador específico?', ph: 'Ej: Fracciones, Sistema Solar, Comprensión lectora...' },
          { id: 'numeroSesiones', label: '4️⃣ ⏳ ¿Cuántas sesiones tienes para este tema?', ph: 'Ej: 3, 5, 8...', type: 'number' as const },
          { id: 'duracionSesion', label: '5️⃣ 🕒 ¿Cuánto dura cada sesión? (minutos)', ph: 'Ej: 45, 60, 90...', type: 'number' as const },
        ].map((f) => (
          <div key={f.id}>
            <label className="flex items-center text-lg font-medium text-gray-800 mb-2">{f.label}</label>
            <input
              type={f.type ?? 'text'}
              value={(respuestas as any)[f.id]}
              onChange={(e) => actualizarRespuesta(f.id, e.target.value)}
              placeholder={f.ph}
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-blue-800 text-sm">
          Generaré la planeación ND adaptada a: <strong>{userData.neurodiversidades.join(', ')}</strong>
        </p>
      </div>

      <div className="flex justify-between items-center pt-4 border-t">
        <button onClick={() => setUserData({ ...userData, paso: 3 })} className="px-6 py-2 text-gray-600 hover:text-gray-800 transition-colors">
          ← Volver
        </button>
        {todasCompletas && (
          <button onClick={continuar} className="px-8 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-medium">
...
/* Paso 4: Recolección Inteligente (solo sensibilidades + verificación) */
function PasoCuatro({ userData, setUserData }: StepProps) {
  ...
  const toggleSensibilidad = (sens: string) => {
    setSensibilidades((prev) => (prev.includes(sens) ? prev.filter((s) => s !== sens) : [...prev, sens]))
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800 mb-3">Recolección Inteligente 🧠</h2>
        <p className="text-gray-600 mb-4">Una pregunta por vez. Validación breve. Evaluación sensorial automática.</p>
      </div>

      <div className="space-y-6">
        <div>
          <h3 className="text-lg font-medium text-gray-800 mb-3">Antes de continuar, ¿hay alguna sensibilidad sensorial que deba considerar?</h3>
          <p className="text-sm text-gray-600 mb-4">(Sonidos, texturas, luces, etc.) Selecciona todas las que apliquen:</p>
            ...
            {sensibilidadesSensoriales.map((sens) => (
              <button
                ...
                className={`p-3 border-2 rounded-lg transition-all duration-200 text-left text-sm ${
                  sensibilidades.includes(sens) ? 'border-orange-400 bg-orange-50 text-orange-800' : 'border-gray-200 hover:border-orange-300'
                }`}
              >
                {sens}
              </button>
        ...
        <div>
          <h3 className="text-lg font-medium text-gray-800 mb-3">¿Incluimos una verificación rápida de habilidades antes de la actividad?</h3>
          <div className="flex gap-4">
            <button
              onClick={() => setVerificacionHabilidades(true)}
              className={`px-6 py-3 border-2 rounded-lg transition-all duration-200 ${
                verificacionHabilidades === true ? 'border-green-400 bg-green-50 text-green-800' : 'border-gray-200 hover:border-green-300'
              }`}
            >
              ✅ Sí, incluir
            </button>
            <button
              ...
              className={`px-6 py-3 border-2 rounded-lg transition-all duration-200 ${
                verificacionHabilidades === false ? 'border-red-400 bg-red-50 text-red-800' : 'border-gray-200 hover:border-red-300'
              }`}
            >
              ❌ No es necesario
            </button>
      ...
      <div className="flex justify-between items-center pt-4 border-t">
        <button onClick={() => setUserData({ ...userData, paso: userData.tipoUsuario === 'docente' && userData.opcionMenu === 'crear' ? 3.5 : 3 })} className="px-6 py-2 text-gray-600 hover:text-gray-800 transition-colors">
          ← Volver
        </button>
        <button onClick={continuar} className="px-8 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-medium">
          Continuar →
...
/* Paso 5: Personalización Avanzada (entornos/limitaciones/prioridad) */
function PasoCinco({ userData, setUserData }: StepProps) {
  ...
  const opcionesEntornos = ['🏠 Casa/Hogar', '🏫 Escuela/Consultorio', '🌍 Espacios públicos', '💼 Trabajo', '🚗 Transporte', '🛒 Centros comerciales']

  const opcionesLimitaciones = ['⏱️ Limitaciones de tiempo', '👥 Otros cuidadores deben implementarla', '💰 Pocos recursos materiales', '📱 Sin acceso a tecnología', '🔇 Entorno ruidoso', '👨‍👩‍👧‍👦 Implementación grupal']

  const toggleEntorno = (entorno: string) => {
    setEntornos((prev) => (prev.includes(entorno) ? prev.filter((e) => e !== entorno) : [...prev, entorno]))
  }

  const toggleLimitacion = (limitacion: string) => {
    setLimitaciones((prev) => (prev.includes(limitacion) ? prev.filter((l) => l !== limitacion) : [...prev, limitacion]))
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800 mb-3">Personalización Avanzada 🎯</h2>
        <p className="text-gray-600 mb-4">Para afinar la propuesta. Responde solo lo que sea relevante.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          <h3 className="text-lg font-medium text-gray-800 mb-3">🔍 ¿Debe funcionar en múltiples entornos?</h3>
          <div className="grid grid-cols-1 gap-2">
            {opcionesEntornos.map((entorno) => (
              <button
                ...
                className={`p-3 border-2 rounded-lg transition-all duration-200 text-left text-sm ${
                  entornos.includes(entorno) ? 'border-blue-400 bg-blue-50 text-blue-800' : 'border-gray-200 hover:border-blue-300'
                }`}
              >
                {entorno}
              </button>
        ...
        <div>
          <h3 className="text-lg font-medium text-gray-800 mb-3">⚠️ Limitaciones o consideraciones especiales</h3>
          <div className="grid grid-cols-1 gap-2">
            {opcionesLimitaciones.map((limitacion) => (
              <button
                ...
                className={`p-3 border-2 rounded-lg transition-all duration-200 text-left text-sm ${
                  limitaciones.includes(limitacion) ? 'border-yellow-400 bg-yellow-50 text-yellow-800' : 'border-gray-200 hover:border-yellow-300'
                }`}
              >
                {limitacion}
              </button>
      ...
      <div>
        <h3 className="text-lg font-medium text-gray-800 mb-3">🎯 ¿Hay alguna prioridad urgente?</h3>
        <textarea
          value={prioridad}
      ...
      <div className="flex justify-between items-center pt-4 border-t">
        <button onClick={() => setUserData({ ...userData, paso: 4 })} className="px-6 py-2 text-gray-600 hover:text-gray-800 transition-colors">
          ← Volver
        </button>
        <button onClick={continuar} className="px-8 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-medium">
          Continuar →
...
function PasoSeis({ userData, setUserData }: StepProps) {
  ...
  const formatos = [
    { id: 'practico', titulo: '🎯 Versión Práctica', subtitulo: 'Lista para usar hoy', descripcion: 'Implementación inmediata con instrucciones claras y directas', tiempo: '⚡ 5-10 minutos de preparación' },
    { id: 'completo', titulo: '⚡ Versión Completa', subtitulo: 'Adaptaciones para diferentes situaciones', descripcion: 'Múltiples variantes y adaptaciones según contexto', tiempo: '📖 15-20 minutos de revisión' },
    { id: 'nd_plus', titulo: '🧠 Versión ND Plus', subtitulo: 'Generalización + capacitación + cronograma', descripcion: 'Incluye formación para cuidadores y plan de implementación extendido', tiempo: '📚 30+ minutos de estudio completo' },
    { id: 'sensorial', titulo: '🌈 Versión Sensorial', subtitulo: 'Enfoque especial en adaptaciones sensoriales', descripcion: 'Diseño específico para sensibilidades y procesamiento sensorial', tiempo: '🎭 Adaptaciones sensoriales detalladas' },
    { id: 'semaforo', titulo: '📊 Versión Semáforo ND', subtitulo: 'Verde = listo, Amarillo = requiere ajuste, Rojo = no viable aún', descripcion: 'Evaluación por fases con indicadores claros de viabilidad', tiempo: '🚦 Sistema de alertas y progreso' },
  ]
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800 mb-3">Selección de Formato 📋</h2>
        <p className="text-gray-600 mb-4">¿Qué formato prefieres para tu planeación ND?</p>
      </div>
        ...
        {formatos.map((formato) => (
          <button
            ...
            className={`p-6 border-2 rounded-xl transition-all duration-200 text-left ${
              formatoSeleccionado === formato.id ? 'border-purple-400 bg-purple-50' : 'border-gray-200 hover:border-purple-300'
            }`}
          >
            <div className="font-bold text-lg text-gray-800 mb-1">{formato.titulo}</div>
            <div className="font-medium text-purple-600 mb-2">{formato.subtitulo}</div>
            <div className="text-sm text-gray-600 mb-3">{formato.descripcion}</div>
            <div className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">{formato.tiempo}</div>
          </button>
        ))}
      </div>
      ...
      {formatoSeleccionado && (
        <div className="flex justify-between items-center pt-4 border-t">
          <button onClick={() => setUserData({ ...userData, paso: 5 })} className="px-6 py-2 text-gray-600 hover:text-gray-800 transition-colors">
            ← Volver
          </button>
          <button onClick={continuar} className="px-8 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-medium">
...
function PasoSiete({ userData, setUserData }: StepProps) {
  const generarPlaneacionCompleta = () => {
    ...
    const comprensionND = {
      explicacion: `Esta actividad honra la neurodiversidad reconociendo que ${userData.neurodiversidades.join(
        ', '
      )} son variaciones naturales del cerebro humano, no déficits a corregir.`,
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-3xl font-bold text-gray-800 mb-3">🎉 ¡Tu Planeación ND está lista!</h2>
        <p className="text-lg text-gray-600 mb-2">Aquí tienes tu propuesta ND para {userData.opcionMenu}</p>
        <p className="text-sm text-purple-600">Honra las fortalezas únicas de {userData.neurodiversidades.join(', ')} con adaptaciones sensoriales</p>
      </div>

      <div className="bg-purple-50 border border-purple-200 rounded-xl p-6">
        <h3 className="text-xl font-bold text-purple-800 mb-4">🧠 SECCIÓN 1: COMPRENSIÓN ND</h3>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
        <h3 className="text-xl font-bold text-blue-800 mb-4">📋 SECCIÓN 2: EVALUACIONES PREVIAS</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <h4 className="font-medium text-gray-800 mb-2">Sensorial:</h4>
            <div className="text-sm text-gray-700">
              {planeacion.evaluaciones.sensorial.length > 0 ? planeacion.evaluaciones.sensorial.map((s) => s.substring(2)).join(', ') : 'Sin sensibilidades reportadas'}
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="text-sm text-gray-700">
              {planeacion.evaluaciones.ambiental.length > 0 ? planeacion.evaluaciones.ambiental.map((e) => e.substring(2)).join(', ') : 'Múltiples entornos'}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-green-50 border border-green-200 rounded-xl p-6">
        <h3 className="text-xl font-bold text-green-800 mb-4">🎯 SECCIÓN 3: IMPLEMENTACIÓN</h3>
      ...
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-xl p-6">
        <h3 className="text-xl font-bold text-purple-800 mb-4">🔄 CIERRE EVOLUTIVO ND</h3>
        <p className="text-gray-700 mb-4">
          Aquí tienes tu propuesta ND para <strong>{userData.opcionMenu}</strong>. Honra las fortalezas únicas de{' '}
          <strong>{userData.neurodiversidades.join(', ')}</strong> y considera adaptaciones sensoriales.
        </p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <button className="p-3 bg-green-100 hover:bg-green-200 border border-green-300 rounded-lg text-sm text-green-800 transition-colors">✅ Implementar tal como está</button>
          <button className="p-3 bg-blue-100 hover:bg-blue-200 border border-blue-300 rounded-lg text-sm text-blue-800 transition-colors">🧠 Ajustar para otra neurodiversidad</button>
          <button className="p-3 bg-purple-100 hover:bg-purple-200 border border-purple-300 rounded-lg text-sm text-purple-800 transition-colors">🌍 Expandir a más entornos</button>
          <button className="p-3 bg-orange-100 hover:bg-orange-200 border border-orange-300 rounded-lg text-sm text-orange-800 transition-colors">👥 Crear material para cuidadores</button>
        </div>
      </div>

      <div className="flex justify-between items-center pt-4 border-t">
        <button onClick={() => setUserData({ tipoUsuario: '', neurodiversidades: [], paso: 1 })} className="px-6 py-3 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors">
          🔄 Nueva planeación
        </button>
        <button onClick={() => setUserData({ ...userData, paso: 6 })} className="px-6 py-2 text-gray-600 hover:text-gray-800 transition-colors">
          ← Cambiar formato
        </button>
      </div>
    </div>
...
function getFortalezasND(neurodiversidades: string[]): string[] {
  ...
  neurodiversidades.forEach((nd) => {
    if (fortalezas[nd]) resultado.push(...fortalezas[nd])
  })
...
function getObjetivoND(userData: UserData): string {
  if (userData.tipoUsuario === 'docente' && userData.grado && userData.temaDetonador) {
    return `Desarrollar ${userData.temaDetonador} en ${userData.grado} honrando las fortalezas de ${userData.neurodiversidades.join(
      ', '
    )} con adaptaciones sensoriales y cognitivas apropiadas.`
  }
...
function getTiempoEstimado(userData: UserData): string {
  if (userData.duracionSesion) return `${userData.duracionSesion} minutos por sesión, distribuidos en fases flexibles con pausas regulares.`
...
function getCapacitacion(userData: UserData): string {
  return `Material informativo para ${userData.tipoUsuario === 'padre' ? 'familia extendida' : 'colegas y cuidadores'} sobre fortalezas de ${userData.neurodiversidades.join(
    ', '
  )} y implementación efectiva.`
}
