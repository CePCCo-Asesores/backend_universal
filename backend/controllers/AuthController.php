...
/**
 * Carga condicional del DatabaseManager.
 * Si utils/database_manager.php no existe en el deploy, definimos un stub mínimo
 * para evitar fatales (los endpoints que usan DB seguirán respondiendo, pero con datos vacíos).
 */
$__dm = __DIR__ . '/../utils/database_manager.php';
if (file_exists($__dm)) {
    require_once $__dm; // define \DatabaseManager real
} else {
    if (!class_exists('\\DatabaseManager', false)) {
        class DatabaseManager {
            public static function fetchOne(string $q, array $p = []): array { return ['count' => 0]; }
            public static function fetchAll(string $q, array $p = []): array { return []; }
...
class AuthController
{
    ...
    public function me(): void
    {
        try {
            ...
            unset($user['google_id']); // no exponer
    ...
    private function exchangeCodeForToken(string $authCode): ?array
    {
        ...
        if ($httpCode !== 200) throw new \Exception("Error obteniendo token de Google: HTTP {$httpCode}");
...
// ===== Alias global (para router que no soporta namespaces)
namespace {
